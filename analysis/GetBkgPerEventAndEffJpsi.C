...
const Double_t massMin = 2.60;
const Double_t massMax = 3.55;
const Double_t nsigma = 3;

Int_t nPtBins = 0;
const Int_t nPtBinsToBeAnalyzed = 8;
Double_t ptBinLimits[nPtBinsToBeAnalyzed] = {0};

const Double_t nEventsBkg = 20e6;   // to be retrieved form the input file!!!
...
TH1D *hBkgPerEvent=0, *hEfficiency=0, *hEfficiencyNoPID=0, *hMassSig[nPtBinsToBeAnalyzed]={0}, *hMassBkg[nPtBinsToBeAnalyzed]={0};
TH2D *hMassVsPtSig=0, *hMassVsPtBkg=0;

TF1 *fitBkg[nPtBinsToBeAnalyzed]={0}, *fitBkgSideBands[nPtBinsToBeAnalyzed]={0}, *fitSig[nPtBinsToBeAnalyzed]={0};
...
void GetBkgPerEventAndEffJpsi(const char* signalfilename = "AnalysisResults_O2_sg_1.44_oina.root",
			      const char* bkgfilename = "AnalysisResults_O2_bkg_1.44_mb.root") {
  
  mystyle();
  BookCanvas();

  const Double_t massJpsi = TDatabasePDG::Instance()->GetParticle(443)->Mass();
  ...
  auto dir_bkg   = (TDirectory*) input_bkg   ->GetDirectory("hf-task-jpsi");

  hMassVsPtSig = (TH2D*) dir_bkg->Get("hmass");
  hMassVsPtSig -> SetName("hMassVsPtSig");

  hMassVsPtBkg = (TH2D*) dir_bkg->Get("hmass");
  ...
  nPtBins = TMath::Min(hMassVsPtBkg->GetNbinsY(), nPtBinsToBeAnalyzed);
  ...
  for (int i = 0; i < nPtBins; i++) {
    ...
    hMassSig[i]->GetXaxis()->SetRangeUser(massMin, massMax);
    hMassBkg[i]->GetXaxis()->SetRangeUser(massMin, massMax);
   ...
   // Setting the fit functions for bkg and sig
    ...
    fitBkg[i]          = new TF1(Form("fitBkg_%d",i),          fitPol,          massMin,massMax,maxPolDegree+1);
    fitBkgSideBands[i] = new TF1(Form("fitBkgSideBands_%d",i), fitPolSideBands, massMin,massMax,maxPolDegree+1);
    fitSig[i]          = new TF1(Form("fitSig_%d",i),"gaus",massJpsi-5*hMassSig[i]->GetRMS(),massJpsi+5*hMassSig[i]->GetRMS());
    ...
    hMassSig[i] -> Fit(fitSig[i],"","",massJpsi-5*hMassSig[i]->GetRMS(),massJpsi+5*hMassSig[i]->GetRMS());
    Double_t sigmaJpsi = fitSig[i]->GetParameter(2);

    sideband[0] = massJpsi - nsigma*sigmaJpsi;
    sideband[1] = massJpsi + nsigma*sigmaJpsi;
    ...
    hMassBkg[i] -> Fit(fitBkgSideBands[i],"","",massMin,massMax);
  ...
  hEfficiency->Write();
  cnvEfficiency->SaveAs("eff.pdf");
  fileOutEff->Close();
 
  TFile *fileOutBkgPerEvents = new TFile("BkgperEvents.root","recreate");
  cnvBkgperEvents -> cd();
